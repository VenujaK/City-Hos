...
<body>
    ...
    if(isset($_GET['search_query'])) {
        ...
        try {
            $sql = "SELECT * FROM `labservice` WHERE LSNAME  LIKE '%$search_query%' ORDER BY RAND() LIMIT 24";
            if ($result = mysqli_query($conn, $sql)) {
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_array($result)) {
                        ...
                        echo '<img src="./uploaded_img/'.$row['LSIMG'].'" class="card-img-top" alt="...">';
                        echo '<div class="card-body">';
                        echo ' <h5 class="card-title">' . $row['LSNAME'] . '</h5>';
                        echo '<p class="card-text">' . $row['LSDES'] . '</p>';
...
?>
    ...
    <?php
    try {
        $sql = "SELECT * FROM `labservice`  ORDER BY RAND() LIMIT 24";
        if ($result = mysqli_query($conn, $sql)) {
            if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_array($result)) {
                    // Display search results
